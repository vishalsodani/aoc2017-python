problem_input = """hwv inc 149 if clj >= -5
...
for row in problem_input:
    row_data = row.split(' ')
    registers[row_data[0]] = 0
    registers[row_data[4]] = 0

for row in problem_input:
    row_data = row.split(' ')
    if row_data[5] == '>':
        if registers[row_data[4]] > int(row_data[6]):
            if row_data[1] == 'inc':
                registers[row_data[0]] += int(row_data[2])
                if registers[row_data[0]] > max_held:
                    max_held = registers[row_data[0]]
            if row_data[1] == 'dec':
                registers[row_data[0]] -= int(row_data[2])
                if registers[row_data[0]] > max_held:
                    max_held = registers[row_data[0]]
